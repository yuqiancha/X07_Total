        ...
        */
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (button1.Text == "连接")
            {
                button1.Text = "断开";
                //得到本机IP，设置UDP端口号     
                IPAddress ip = getValidIP(textBox_LocalIP.Text);
                int port = getValidPort(textBox_LocalPort.Text);
                ...
                //得到客户机IP  
                IPAddress ip2 = getValidIP(textBox_RemoteIP.Text);
                int port2 = getValidPort(textBox_RemotePort.Text);
                IPEndPoint ipep = new IPEndPoint(ip2, port2);
                ...
                //开启发送线程，以便在EPDU中解析到APID时，推送到队列中，发送线程读取队列中数据再发出
                new Thread(() => { Send_2_UDPServer(); }).Start();
            }
            else
            {
                button1.Text = "连接";
        ...
        private void btn_UDP_Send_Click(object sender, EventArgs e)
        {

            byte[] data = StrToHexByte(this.textBox_UDP_SendTxt.Text);
        ...
        private void Send_2_UDPServer()
        {
            while(RunningFlag)
            {
                if(Data.DataQueue_UDP2Server.Count>0)
                {
                    byte[] TempBuf = Data.DataQueue_UDP2Server.Dequeue();
                    int Len = TempBuf.Length;//取出的数据区实际长度
                    byte[] SendBuf = new byte[7 + Len];
                    SendBuf[0] = 0x1A;
                    SendBuf[1] = 0xCF;
                    SendBuf[2] = 0xFC;
                    SendBuf[3] = 0x1D;
                    SendBuf[4] = (byte)(Len -1);//帧格式中数据区长度需要-1
                    Array.Copy(TempBuf, 0, SendBuf, 5, Len);

                    byte[] MultByte = new byte[Len + 1];//需要校验的是数据区长度+数据区
                    MultByte[0] = SendBuf[4];
                    Array.Copy(TempBuf, 0, MultByte, 1, Len);
                    byte[] HJY = Function.Multiware(MultByte);

                    SendBuf[Len + 5] = HJY[0];
                    SendBuf[Len + 6] = HJY[1];

                    mySocket.SendTo(SendBuf, SendBuf.Length, SocketFlags.None, RemotePoint);
                }
        ...
        private int getValidPort(string port)
        {
            ...
            catch (Exception e)
            {
                ...
                //FormatException,   
                //OverflowException  
                Trace.WriteLine("无效的端口号：" + e.ToString());
                this.textBox_UDP_RecvTxt.AppendText("无效的端口号：" + e.ToString() + "\n");
                return -1;
            }
            return lport;
        }
        ...
        private IPAddress getValidIP(string ip)
        {
            ...
            catch (Exception e)
            {
                ...
                //FormatException,   
                //OverflowException  
                Trace.WriteLine("无效的IP：" + e.ToString());
                this.textBox_UDP_RecvTxt.AppendText("无效的IP：" + e.ToString() + "\n");
                return null;
            }
            return lip;
        }
        ...
        private void ReceiveHandle()
        {
            ...
            while (RunningFlag)
            {
                ...
                try
                {
                    ...
                    msg = "";
                    for (int i = 0; i < rlen; i++) msg += data[i].ToString("x2");
                    textBox_UDP_RecvTxt.BeginInvoke(myI, new object[] { RemotePoint.ToString() + " : " + msg });
                }
                catch(Exception ex)
                {
                    Trace.WriteLine(ex.Message);
        ...
        {
            //接收数据显示  
            if (this.textBox_UDP_RecvTxt.Lines.Count() > 20) textBox_UDP_RecvTxt.Clear();
            this.textBox_UDP_RecvTxt.AppendText(msg + "\n");
        }
    }
}
